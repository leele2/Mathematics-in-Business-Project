...
A Wiener process in statistics is a continuous-time stochastic process named after Norbert Wiener for his study off the mathematical properties of one-dimension Brownian motion.

A Wiener process, \( \wei_t \) is characterized by the following properties:
\begin{enumerate}
    \item \( \wei_0 \) = 0
    ...
    \item \( \wei \) has Gaussian increments: \( \wei_{t+u} - \wei_t \) is normally distributed with mean 0 and variance \( u \), i.e. \( \wei_{t+u} - \wei_t \sim \norm\p{0, u}\).
    \item \( \wei \) has continuous paths: \( \wei_t \) is continuous in \( t \).
\end{enumerate}

\subsubsection{Asset Prices as Random Processes}
