...
A random walk is a type of stochastic process that describes a path in which at every movement forward in time the movement in space is decided by a random process. For example, in the 2 dimension case, we can construct a path in which we start at the 0 and for every time step forward we flip a fair coin to decide if we move up or down. We let \( S_i \) be our position of the path at time \(t = t_i\) and \(y_i\) be the result of our \(i^\text{th}\) coin toss and \(Y_i\) be how the toss effects our path; where \(Y_i = 1\) if \( y_i = \text{heads} \) and \(Y_i = -1\) if \(y_i = \text{tails}\). If we were to plot our path it would look something like;
...
It follows that we can construct a random walk for the general case we may change the probabilities, the size of the step taken and also introduce a drift parameter. This is written as follows:
